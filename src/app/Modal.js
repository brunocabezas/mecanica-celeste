      ...
      <div ref={"app__modal"}>
        <ReactModal {...modalProps}>
          ...
          <div className="app__modal-info">
            ...
            <h4>{data.acf.anio} {data.acf.lugar && "- "+data.acf.lugar} </h4>
            <h5>Autores: {data.acf.nombre} {data.acf.profesion && "- "+data.acf.profesion} </h5>
            <p className={"app__modal-abstract"}
               dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(data.acf.resena)}} />
          </div>
        </ReactModal>
      </div>;
  }
